...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (!user) return;
    const q = query(collection(db, 'locations'), where('userId', '==', user.uid), orderBy('start', 'desc'));
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="card">
        {editing ? (
          ...
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-indigo-50 border border-indigo-200 flex items-center justify-center text-3xl flex-shrink-0">
              {profile?.emoji || '👤'}
            </div>
            <div className="flex-1 min-w-0">
              <h2 className="font-display font-bold text-xl text-stone-900">{profile?.name}</h2>
      ...
      {!editing && (
        <div className="grid grid-cols-2 gap-3">
          <div className="card text-center py-4">
            ...
            <p className="text-xs text-stone-400">Aniversário</p>
            {profile?.birthDate ? (
              <>
                <p className="font-semibold text-stone-800 text-sm mt-0.5">
                  {format(new Date(profile.birthDate.replace(/-/g, '/')), "d 'de' MMM", { locale: ptBR })}
                </p>
                <p className="text-xs text-stone-400">{age} anos</p>
              </>
            ) : <p className="text-xs text-stone-400 mt-0.5">Não informado</p>}
          </div>
          <div className="card text-center py-4">
            <Shield size={20} className="mx-auto text-indigo-500 mb-1" />
            <p className="text-xs text-stone-400">Perfil de acesso</p>
            <p className="font-semibold text-stone-800 text-sm mt-0.5 capitalize">{profile?.role}</p>
            <p className="text-xs text-stone-400">{profile?.department}</p>
          </div>
        </div>
      )}
      ...
      <div className="card">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-semibold text-stone-800">Histórico de Localização</h3>
        </div>
        {locations.length === 0 ? (
          <p className="text-stone-400 text-sm text-center py-3">Sem registros de localização</p>
        ) : (
          <div className="space-y-2">
            {locations.map(loc => (
              <div key={loc.id} className="flex items-center gap-3 py-1.5 border-b border-stone-100 last:border-0">
                <div className="w-2 h-2 rounded-full bg-indigo-400 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-stone-700">{loc.location}</p>
