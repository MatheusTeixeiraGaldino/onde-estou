...
import {
  ...
  eachDayOfInterval, getDay, addMonths, subMonths, parseISO
} from 'date-fns';
...
export default function EventsPage() {
  ...
  // Open modal — optionally pre-fill date from calendar click
  const openCreate = (day = null) => {
    const prefillDate = day
      ? format(day, "yyyy-MM-dd'T'09:00")
      : '';
    setForm({ title: '', description: '', date: prefillDate, location: '', category: 'Reunião' });
    setShowModal(true);
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setShowModal(false);
      // Select the new event's day on the calendar
  ...
  /* ───── EVENT CARD (used in both list and sidebar) ───── */
  const EventCard = ({ ev, compact = false }) => {
    ...
    return (
      <div className={`card-hover ${compact ? 'p-3' : ''}`}>
        ...
        {!compact && (
          <p className="text-xs text-stone-300 mt-2">por {ev.creatorName}</p>
        )}
      </div>
    );
  };
        ...
        <>
          <div className="card p-0 overflow-hidden">
              ...
              {days.map(day => {
                ...
                return (
                  <button
                    ...
                    title="Clique para ver • Duplo-clique para criar evento"
                    className={`
                      relative aspect-square rounded-xl flex flex-col items-center justify-start pt-1.5 text-sm transition-all group
                    ...
                    {dayEvts.length > 0 && (
                      <div className="flex gap-0.5 mt-1 flex-wrap justify-center px-0.5">
                        {dayEvts.slice(0, 3).map((ev, i) => (
                          <div
                            key={i}
                            className={`w-1.5 h-1.5 rounded-full ${today ? 'bg-white/70' : (CATEGORY_DOT[ev.category] || 'bg-stone-400')}`}
                          />
                        ))}
                      </div>
                    )}
            ...
            <div className="flex items-center gap-4 px-5 py-3 border-t border-stone-100 flex-wrap">
              {Object.entries(CATEGORY_DOT).slice(0, 4).map(([cat, dot]) => (
          ...
          {/* All upcoming in this month (small list below) */}
          {!selectedDay && (
            <div className="space-y-2">
              {events
                .filter(e => {
                ...
                .map(ev => (
                  <div
                    key={ev.id}
                    onClick={() => setSelectedDay(new Date(ev.date))}
        ...
        <Modal title="Novo Evento" onClose={() => setShowModal(false)}>
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea className="input min-h-[70px] resize-none" placeholder="Detalhes do evento..." value={form.description} onChange={e => set('description', e.target.value)} />
            </div>
            ...
            <div className="flex items-center gap-2 p-3 bg-stone-50 rounded-xl border border-stone-100">
              <div className={`w-2.5 h-2.5 rounded-full ${CATEGORY_DOT[form.category] || 'bg-stone-400'}`} />
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${CATEGORY_COLORS[form.category] || CATEGORY_COLORS['Outro']}`}>
                {form.category}
              </span>
              {form.date && (
                <span className="text-xs text-stone-400 ml-auto">
                  {format(new Date(form.date), "d 'de' MMM 'às' HH:mm", { locale: ptBR })}
                </span>
              )}
            </div>
